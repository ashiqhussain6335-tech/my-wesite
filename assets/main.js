...
if(canvas){
  ...
  class Particle {
    ...
    draw(){
      ...
      ctx.fillStyle = `rgba(62, 207, 110, ${this.alpha})`;
      ctx.fill();
    }
  }
    ...
    for(let i = 0; i < particles.length; i++){
      for(let j = i + 1; j < particles.length; j++){
        ...
        if(dist < 150){
          ...
          ctx.strokeStyle = `rgba(62, 207, 110, ${0.06 * (1 - dist/150)})`;
          ctx.lineWidth = 0.5;
          ctx.stroke();
        }
      }
...
if(nav){
  window.addEventListener('scroll', ()=>{
    nav.style.background = window.scrollY > 100 ? 'rgba(14,22,18,0.95)' : 'rgba(14,22,18,0.85)';
  }, {passive: true});
}
